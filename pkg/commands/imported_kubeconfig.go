// Code generated by go run tools/import_commands.go --talos-version v1.10.4 kubeconfig
// DO NOT EDIT.
...
func init() {
	kubeconfigCmd.Flags().StringSliceVarP(&kubeconfigCmdFlags.configFiles,
		"file", "f", nil, "specify config files or patches in a YAML file (can specify multiple)",
	)
	kubeconfigCmd.PreRunE = func(cmd *cobra.Command, args []string) error {
		nodesFromArgs := len(GlobalArgs.
			Nodes) > 0

		endpointsFromArgs := len(GlobalArgs.
			Endpoints) > 0
		for _, configFile := range kubeconfigCmdFlags.
			configFiles {
